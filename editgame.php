...
<body>
	<?php
		...
		if(isset($_GET['idupdate'])){
			$idgame = $_GET['idupdate'];

			$game = new Game();
			$row = $game->getGameById($idgame);
		}
	?>
<div class="nav-container">
	...
	<form method = "post" id = "form" enctype = "multipart/form-data" class="form-small">
		...
		<input type="hidden" id = "idgame" name="idgame" value="<?php echo($idgame); ?>">
		<p>
			<label>Game Name</label><input type = "text" name = "name" id = "name" value = "<?php echo($row['name']) ?>" required>
		</p>
		<p>
			...
			$.ajax({
				...
				success: function(response){
            		...
            		if (response.trim() === "berhasil") {
                		window.location.href = "gamelist.php?edit=1";
            		}
            		else{
            			window.location.href = "gamelist.php?edit=0";
            		}
				},
